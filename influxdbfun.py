...
def save_data_to_influx(data: SenML):
    ...
    for record in data.e:
        try:
            feature_name = record.n[0]
            if record.v is None or not isinstance(record.v, (int, float)):
                continue

            point = (
                Point("Sensor_data")
                .tag("sensor", record.bn or "unknown")
                .tag("user_id", user_id)
                .tag("execution_id", record.execution_id or "none")
                .field(feature_name, float(record.v))
                .field("unit", record.u or "")
                .time(record.t or datetime.utcnow())
            )
            points.append(point)
...
def save_model_output_to_influx(sensor: str, output, user_id: str = "anonymous", execution_id: str = "none", model_name: str = "unknown"):
    try:
        ...
        point = (
            ...
            .field("result", json.dumps(output.tolist()))
